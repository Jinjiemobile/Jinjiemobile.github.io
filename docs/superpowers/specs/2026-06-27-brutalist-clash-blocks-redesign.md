...
Hero 区域加极淡网格线 `rgba(255,255,255,0.04)`,间距 32px,增加"工程蓝图"感。
...
- **深色为默认**(初始 `data-theme="dark"`,与 index.html 现有 `data-theme="light"` 相反)
- 浅色主题同样应用所有 Brutalist token(浅底 `#fafaf7` + 撞色块),阴影颜色对比度保持有效
- 切换逻辑保留现有 `script.js`,token 值在两个 theme 下分别定义
